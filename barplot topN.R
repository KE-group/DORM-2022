...
resDF <- resDF[order(-resDF$counts,
                     resDF$Gene,
                     resDF$Mutation,
                     decreasing = F),]
resDF$mutsID <- paste(resDF$Gene,
                      resDF$Mutation,
                      sep="_")
...
ggplot(data=topN,aes(x=reorder(mutsID,-counts),
                     y=counts))+
  geom_col(fill="#00abea",
           color=NA)+
  ylab("Number of somatic mutations")+
  xlab(paste0("Mutations (n=", length(topN$mutsID), ")"))+
...
write.table(x = topN[1:20,1:4],
            file ="~/Desktop/tmp.csv",
            sep=",",
            row.names = F,
            col.names = T)
...
ggplot(data=topN,
       aes(x=reorder(mutsID,-counts),
           y=counts))+
  geom_col(fill="#ea3f00",color=NA)+
  ylab("Number of somatic mutations")+
  xlab(paste0("Mutated residues (n=",length(topN$mutsID),")"))+
  scale_y_continuous(limits = c(0,max(topN$counts)),
